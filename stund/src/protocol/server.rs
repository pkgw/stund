...
pub type Ser = WriteJson<FramedWrite<WriteHalf<UnixStream>>, ServerMessage>;
pub type De = ReadJson<FramedRead<ReadHalf<UnixStream>>, ClientMessage>;
...
impl Server {
    pub fn launch() -> Result<i32, Error> {
        let p = get_socket_path()?;
        ...
        let log: Box<Write> = if true /*opts.foreground*/ {
    ...
    fn handle_open(&mut self, conn: &mut UnixStream, params: OpenParameters) -> Result<(), Error> {
        ...
        let mut ptymaster = pseudotty::create().context("failed to create PTY")?;
